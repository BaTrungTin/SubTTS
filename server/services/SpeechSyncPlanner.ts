...
export interface SyncPlanItem {
  ...
  adjustedTtsPath?: string;
}

export interface SyncPlan {
  mode: 'video-priority' | 'hybrid';
  items: SyncPlanItem[];
  gaps: Array<{ originalStart: number; duration: number; outputStart: number }>;
  finalGap?: { originalStart: number; duration: number; outputStart: number };
  totalOutputDuration: number;
  stats: {
    stretchedSegments: number;
    spedUpSegments: number;
    overlapFixes: number;
  };
}
...
export function buildSyncPlan(
  ...
  for (let i = 0; i < subtitles.length; i++) {
    ...
    if (mode === 'hybrid') {
      ...
      if (videoStretchFactor > 1.01) stretchedSegments++;
...
export function attachFinalGap(plan: SyncPlan, videoDuration: number): SyncPlan {
  const lastOriginalEnd =
    plan.items.length > 0
  ...
  if (videoDuration > lastOriginalEnd + 0.001) {
    ...
    return {
      ...
      totalOutputDuration:
        plan.mode === 'hybrid' ? plan.totalOutputDuration + duration : videoDuration,
    };
  }
