...
class GeminiTranslator {
    ...
    static estimateQuota(linesRemaining, batchSize) {
        const size = batchSize ?? GeminiTranslator.computeBatchSizeFromLineCount(linesRemaining);
    ...
    computeBatchSize(subtitles, userOverride) {
        ...
        return GeminiTranslator.computeBatchSizeFromLineCount(subtitles.length, avgChars, userOverride);
    }
    isDailyQuotaExhausted(info, message) {
        const m = message.toLowerCase();
        return (info.googleStatus === 'RESOURCE_EXHAUSTED' ||
            m.includes('per day') ||
            m.includes('daily') ||
            m.includes('rpd') ||
    ...
    extractTextFromResponse(data) {
        ...
        const text = (candidate.content?.parts || [])
            ...
            .trim();
        if (!text) {
            throw new Error(`Phản hồi rỗng từ Gemini (finishReason: ${finish || 'UNKNOWN'})`);
        }
        return text;
    }
    sleep(ms) {
    ...
    async translateSrt(srtContent, apiKeys, onProgress, callbacks = {}, batchSizeOverride) {
        ...
        const translatedSubtitles = subtitles.map((sub, index) => {
            ...
            if (existing && (0, srtUtils_1.isValidTranslation)(sub.text, existing)) {
                return { ...sub, text: existing };
            }
            return { ...sub, text: '' };
        });
        const hasValidTranslationAt = (index) => (0, srtUtils_1.isValidTranslation)(subtitles[index].text, existingTranslations[index] || translatedSubtitles[index]?.text);
        ...
        onProgress(0.08, `Gói free: ~${FREE_TIER_RPD_PER_MODEL} request/ngày/model × ${quotaEst.modelsAvailable} model ≈ ${quotaEst.maxLinesPerDayApprox} dòng/ngày. ` +
        ...
        const worker = async (workerId) => {
            ...
            while (pendingBatches.length > 0 && !stoppedForQuota) {
                ...
                const progressPercent = 0.1 + 0.8 * ((alreadyDone + (completedBatches / totalPending) * (subtitles.length - alreadyDone)) / subtitles.length);
                onProgress(progressPercent, `Translating lines ${batchIndices[0] + 1} to ${batchIndices[batchIndices.length - 1] + 1} / ${subtitles.length}...`);
                try {
                    while (exhaustedModels.has(GEMINI_MODELS[batchModelIndex % GEMINI_MODELS.length]) &&
                        exhaustedModels.size < GEMINI_MODELS.length) {
                        batchModelIndex++;
                    }
                    const result = await this.translateBatchWithRotation(batchLines, normalizedKeys, currentKeyIndex, batchModelIndex % GEMINI_MODELS.length, exhaustedModels, (logMsg) => onProgress(progressPercent, logMsg));
                    ...
                    const quotaHit = exhaustedModels.size >= GEMINI_MODELS.length ||
                        /quota|429|RESOURCE_EXHAUSTED|hết quota|per day|rpd/i.test(msg);
                    if (quotaHit) {
                        stoppedForQuota = true;
                        pendingBatches.unshift(item);
                        onProgress(progressPercent, `⏸ Hết quota Gemini hôm nay (~${FREE_TIER_RPD_PER_MODEL} request/ngày/model). ` +
                    ...
                    if (retries < 4) {
                        ...
                        onProgress(progressPercent, `[LỖI] Batch dòng ${batchIndices[0] + 1} (thử lại ${retries}/3): ${msg}`);
                        await this.sleep(4000);
                    }
                    else {
                        onProgress(progressPercent, `[BỎ QUA] Batch dòng ${batchIndices[0] + 1} sau 3 lần lỗi: ${msg}`);
                    }
                }
                completedBatches++;
            }
        };
        const workers = Array.from({ length: Math.min(concurrency, totalPending) }, (_, idx) => worker(idx));
        await Promise.all(workers);
        const result = this.buildTranslateResult(subtitles, translatedSubtitles, stoppedForQuota);
        onProgress(result.isComplete ? 0.95 : 0.9, result.isComplete
    ...
    buildTranslateResult(subtitles, translatedSubtitles, quotaExhausted) {
    ...
    async translateBatchWithRotation(texts, apiKeys, startKeyIndex, startModelIndex, exhaustedModels, logRotation) {
        ...
        while (attempts < maxAttempts) {
            ...
            if (exhaustedModels.has(model)) {
                ...
                if (exhaustedModels.size >= GEMINI_MODELS.length) {
                    throw new Error(`Đã hết quota ngày (RPD) cho tất cả model. Free tier ~${FREE_TIER_RPD_PER_MODEL} request/ngày/model. ` +
            ...
            try {
                ...
                if (this.isDailyQuotaExhausted(info, info.message)) {
                    ...
                    logRotation(`[INFO] Model ${model} hết quota ngày (~${FREE_TIER_RPD_PER_MODEL} RPD). Chuyển model khác...`);
                    modelIndex = (modelIndex + 1) % GEMINI_MODELS.length;
                    continue;
                }
        ...
        const has429 = errorLog.some((e) => e.includes('429') || e.includes('RESOURCE_EXHAUSTED'));
        if (has429 || exhaustedModels.size > 0) {
            throw new Error(`Hết quota/rate limit. Free tier: ~${FREE_TIER_RPD_PER_MODEL} request/ngày/model (không phải 10 req tổng). ` +
                `Chờ reset hoặc bấm Tiếp tục dịch ngày mai. Chi tiết: ${recent}`);
        }
        throw new Error(`Không gọi được Gemini API. Kiểm tra key tại https://aistudio.google.com/apikey và bật Generative Language API. Chi tiết: ${recent}`);
    }
    parseSrt(srtContent) {
