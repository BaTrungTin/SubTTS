...
const AudioEngine_1 = require("./services/AudioEngine");
class RenderingManager {
    constructor() {
        this.tts = new TtsService_1.TtsService();
        this.audio = new AudioEngine_1.AudioEngine();
    }
    async processDubbing(params, onProgress) {
        try {
            ...
            for (let i = 0; i < totalSubs; i++) {
                ...
                onProgress(0.1 + 0.4 * (i / totalSubs), `Synthesizing voice lồng tiếng for sub #${i + 1}/${totalSubs}... [${setting.provider}]`);
                const clipPath = await this.tts.generateSpeech(sub.text, setting.provider, setting.voice, {
                ...
                if (userSpeed !== 1.0 && userSpeed > 0) {
                    const adjustedPath = path_1.default.join(os_1.default.tmpdir(), 'vibe-tts-clips', `speed-${i}-${Date.now()}.mp3`);
            ...
            const finalVideoPath = await this.audio.renderFinalVideo(params.videoPath, params.subtitles, audioClips, params.mixing, params.mode, params.outputPath, onProgress);
